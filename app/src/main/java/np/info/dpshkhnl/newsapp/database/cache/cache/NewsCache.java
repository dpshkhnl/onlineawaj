...
package np.info.dpshkhnl.newsapp.database.cache.cache;

import android.database.Cursor;
import android.os.Handler;

import np.info.dpshkhnl.newsapp.database.cache.BaseCache;
...
import np.info.dpshkhnl.newsapp.support.sax.SAXNewsParse;
...
public class NewsCache extends BaseCache<NewsBean> {
    ...
    protected void putData() {
        ...
        for(int i=0;i<mList.size();i++){
          ...
          values.put(NewsTable.IMAGE,newsBean.getImage());
    ...
    public synchronized void loadFromCache() {
        ...
        while (cursor.moveToNext()) {
            ...
            newsBean.setLink(cursor.getString(NewsTable.ID_LINK));
             newsBean.setImage(cursor.getBlob(NewsTable.ID_IMAGE));
